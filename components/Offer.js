...
export default class Offer extends React.PureComponent {
	...
	getOffers() {
		fetch(
			BASE_URL +
				`api/offers?country=${this.state.country_id}&region=${this.state.region_id}`
		)
			.then((response) => response.json())
			.then((responseJson) => {
				// alert(JSON.stringify(responseJson));
				if (responseJson.success) {
                    this.setState({offers:responseJson.data.offers})
					// if (responseJson.data.length > 0) {
					// 	let speciality_data = {};
					// 	let specialityList = [];
					// 	responseJson.data.forEach((item) => {
	...
	renderItem = ({item}) => (
		<TouchableOpacity
			onPress={() => {
				this.props.navigation.navigate("OfferDetail", {offer_id: item.id,name:item.title});
			}}
			key={item.id}
			style={{
				marginVertical: 4,
